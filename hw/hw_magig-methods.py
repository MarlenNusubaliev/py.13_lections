...
class Notebook:
    def __init__(self, ram: int, memory: int, cpu: int) -> None:
        self.ram = ram
        self.memory = memory
        self.cpu = cpu
    ...
    @classmethod
    def add_notebook(cls, nodebook_dict):
        return cls(nodebook_dict['ram'], nodebook_dict['memory'], nodebook_dict['cpu'])
...
new_nodebook = Notebook.add_notebook(nodebook_dict)
print(new_nodebook.info())